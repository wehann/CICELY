...
def get_start_end_code(tools_directory, instruction):
    start_code, end_code = '', ''
    cmd = tools_directory + 'symbol-collect-user-64 -d page -D logs_out/page.log -- ' + instruction
    ...
    if start_code == '' and end_code == '':
        cmd = tools_directory + 'symbol-collect-user-32 -d page -D logs_out/page.log -- ' + instruction
        ex = subprocess.Popen(cmd, stdout=subprocess.PIPE, shell=True)
        out, err = ex.communicate()
        ex.wait()
        for line in open('logs_out/page.log', 'r'):
...
def start(configs, work_folder):
    ...
    tools_directory = work_folder + os.path.sep + '..' + os.path.sep + 'tools' + os.path.sep
    ...
    for project_name, config in configs.items():
        ...
        for i in range(len(input_directories)):
            ...
            for input_filename in input_files:
                ...
                start_code, end_code = '', ''
                start_code, end_code = get_start_end_code(tools_directory, execute_instruction)
